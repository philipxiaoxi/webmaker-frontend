<template>
    <div class="container">
        <big-title class=" px-20">扩展功能</big-title>
        <div class="link-container grid-cols-3 gap-sm">
            <link-icon
            v-for="(item,i) in extendsList" :key="item.title + i"
            :title="item.title"
            :intro="item.intro"
            :url="item.url"
            :type="item.type"
            :img='item.img'
            ></link-icon>
        </div>
        <big-title class=" px-20">前端工具箱</big-title>
        <div class="link-container grid-cols-3 gap-sm">
            <link-icon
            v-for="(item,i) in linksList" :key="item.title + i"
            :title="item.title"
            :intro="item.intro"
            :url="item.url"
            :img='item.img'
            ></link-icon>
        </div>
        <big-title class=" p-20">创意工厂</big-title>
        <creative-factory></creative-factory>
    </div>
</template>

<script>
import BigTitle from '../components/BigTitle.vue'
import CreativeFactory from '../components/CreativeFactory.vue'
import LinkIcon from '../components/LinkIcon.vue'
export default {
    components: { LinkIcon, BigTitle, CreativeFactory },
    data() {
        return {
            linksList: [
                {
                    title: '获取漂亮的边框',
                    intro: 'All of these box-shadow were copied using.',
                    url: 'https://getcssscan.com/css-box-shadow-examples?ref=producthunt',
                    img: 'https://toastlog.com/img/logos/cssscan.svg'
                },
                {
                    title: '获取随机字符串和图片',
                    intro: 'API tool for generating lorem ipsum, and getting placeholder images.',
                    url: 'https://sdfsdf.dev/'
                },
                {
                    title: '在线base64解码/编码工具',
                    intro: '在线base64解码/编码工具是一个可以将字符串进行base64解码/编码。',
                    url: 'https://c.runoob.com/front-end/693/',
                    img: 'data:image/png;base64,'
                },
                {
                    title: '免费SSL证书',
                    intro: 'FreeSSL.cn 是一个免费提供 HTTPS 证书申请。',
                    url: 'https://freessl.cn/login'
                },
                {
                    title: '正则表达式在线测试',
                    intro: '在线进行正则表达式在线测试。',
                    url: 'https://c.runoob.com/front-end/854/',
                    img: 'data:image/png;base64,'
                },
                {
                    title: 'web前端学习架构',
                    intro: '聚集一切前端资源的导航系统。',
                    url: 'http://www.alloyteam.com/nav/'
                },
                {
                    title: '同态生成器',
                    intro: 'Generate Soft-UI CSS code',
                    url: 'https://neumorphism.io/#e0e0e0'
                },
                {
                    title: '色彩空间',
                    intro: 'free collection of 180 linear gradients.',
                    url: 'https://webgradients.com/',
                    img: 'https://webgradients.com/img/other/logos/webgradients_logo.png'
                },
                {
                    title: '免费音频素材',
                    intro: '113万首音频供您下载。',
                    url: 'https://www.aigei.com/sound'
                }
            ],
            extendsList: [
                {
                    title: 'WebMaker低代码平台',
                    intro: '拖拽生成网页、自定义引入第三方组件、智能解析网页元素并合并、生成原生HTML代码。',
                    url: '/lowcode',
                    type: 'path',
                    img: 'https://toastlog.com/img/logos/cssscan.svg'
                }
            ]
        }
    }
}

</script>

<style lang='less' scoped>
.container {
    max-width: 1144px;
    padding: 20px;
    margin: 0 auto;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 10px 50px;
    margin-top: 80px;
    margin-bottom: 20px;
}
.link-container {
    margin-bottom: 20px;
}
</style>
