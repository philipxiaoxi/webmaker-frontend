export const appList = [
    {
        id: 1,
        title: '获取漂亮的边框',
        intro: '漂亮的 CSS 框阴影示例所有这些盒影都是免费使用。使用CSS扫描，您可以轻松检查或复制任何网站的CSS。',
        url: 'https://getcssscan.com/css-box-shadow-examples?ref=producthunt',
        type: 'url',
        img: 'https://toastlog.com/img/logos/cssscan.svg',
        category: '实用工具',
        tags: [
            { type: 'success', content: '绿色认证' }
        ]
    },
    {
        id: 2,
        title: '获取随机字符串和图片',
        intro: '有一天，我梦想着自己为前端开发人员创建了一个网站，这是一个用于生成lorem ipsum（又名假词）和获取占位符图像的API工具。梦甚至告诉我网站的域名。醒来后，我搜索了域名可用性，它可用！因此，我决定创建这个网站，并将整个开发过程记录为截屏视频，可以在这里找到。',
        url: 'https://sdfsdf.dev/',
        type: 'url',
        category: '实用工具',
        tags: [
            { type: 'success', content: '绿色认证' }
        ]
    },
    {
        id: 3,
        title: '在线base64解码/编码工具',
        intro: '在线base64解码/编码工具是一个可以将字符串进行base64解码/编码。',
        url: 'https://c.runoob.com/front-end/693/',
        type: 'url',
        img: 'data:image/png;base64,',
        category: '实用工具',
        tags: [
            { type: 'success', content: '绿色认证' }
        ]
    },
    {
        id: 13,
        title: '免费SSL证书',
        intro: 'FreeSSL.cn 是一个免费提供 HTTPS 证书申请、HTTPS 证书管理和 HTTPS 证书到期提醒服务的网站，旨在推进 HTTPS 证书的普及与应用，简化证书申请的流程。',
        url: 'https://freessl.cn/login',
        type: 'url',
        category: '程序员集',
        tags: [
            { type: 'success', content: '绿色认证' }
        ]
    },
    {
        id: 4,
        title: '正则表达式在线测试',
        intro: '在线进行正则表达式在线测试。',
        url: 'https://c.runoob.com/front-end/854/',
        type: 'url',
        img: 'data:image/png;base64,',
        category: '实用工具',
        tags: [
            { type: 'success', content: '绿色认证' }
        ]
    },
    {
        id: 5,
        title: 'web前端学习架构',
        intro: '聚集一切前端资源的导航系统。',
        type: 'url',
        url: 'http://www.alloyteam.com/nav/',
        category: '程序员集'
    },
    {
        id: 6,
        title: 'web前端导航',
        intro: '聚集一切前端资源的导航系统。左下角看板娘，可以切换人物和衣服，切换时加载较慢，可能需要1分钟，耐心等待是有惊喜的！',
        url: 'http://5cv.top',
        type: 'url',
        category: '程序员集'
    },
    {
        id: 7,
        title: '同态生成器',
        intro: '为您的设计生成软UI CSS代码。这对于创建 Neumorphism 设计非常有用。您可以选择颜色，编辑大小，半径，距离等等。',
        url: 'https://neumorphism.io/#e0e0e0',
        type: 'url',
        category: '实用工具'
    },
    {
        id: 8,
        title: '色彩空间',
        intro: '免费收集180个线性梯度的色彩供您选择。',
        url: 'https://webgradients.com/',
        type: 'url',
        img: 'https://webgradients.com/img/other/logos/webgradients_logo.png',
        category: '实用工具'
    },
    {
        id: 9,
        title: '免费素材',
        intro: '免费素材  高效创作作为国内专业的数字娱乐创作者服务平台，爱给网（aigei.com）致力于为声音、影视、游戏、动画创作者，提供高品质、多品类、优秀的创意作品服务。',
        url: 'https://www.aigei.com/sound',
        type: 'url',
        category: '程序员集'
    },
    {
        id: 10,
        title: '咸鱼云网盘',
        type: 'insideApp',
        intro: '咸鱼云网盘是一款在线不限速的开源网盘，拥有丰富的资源，后期将会与webmaker账户互通。',
        url: 'https://disk.xiaotao2333.top:344/#/public',
        img: 'https://disk.xiaotao2333.top:344/api/static/defaultAvatar.png',
        category: '推荐应用',
        auth: 'token',
        tags: [
            { type: 'success', content: '绿色认证' },
            { type: 'danger', content: '账户不互通' }
        ]
    },
    {
        id: 11,
        title: 'webmaker虚拟容器',
        intro: 'webmaker虚拟容器，在这里仅作微应用加载测试，在本站中点击导航也可以访问。',
        url: 'https://docker.diyxi.top/#/',
        type: 'insideApp',
        img: 'https://disk.xiaotao2333.top:344/api/static/defaultAvatar.png',
        category: '推荐应用',
        tags: [
            { type: 'success', content: '绿色认证' },
            { type: 'success', content: '账户互通' },
            { type: 'danger', content: '测试版本' }
        ]
    },
    {
        id: 12,
        title: 'WebMaker低代码平台',
        intro: '拖拽生成网页、自定义引入第三方组件、智能解析网页元素并合并、生成原生HTML代码。',
        url: '/lowcode',
        type: 'path',
        img: 'https://toastlog.com/img/logos/cssscan.svg',
        category: '推荐应用',
        tags: [
            { type: 'success', content: '绿色认证' },
            { type: 'success', content: '账户互通' }
        ]
    },
    {
        id: 16,
        title: '小睡眠组件',
        intro: '小睡眠官方测试组件，拥有超过500+的音频资源，能够随时随地进行放松与聆听。',
        img: 'https://tse4-mm.cn.bing.net/th/id/OIP-C.qCxPmaVo2p-u_Z89dcIitAHaHa?w=183&h=183&c=7&r=0&o=5&pid=1.7',
        type: 'component',
        category: '小组件(测试)',
        url: 'https://www.xiaotao2333.top:5885/common/SnippetHtml/178'
    },
    {
        id: 17,
        title: '小时钟组件',
        intro: '小时钟官方测试组件，一个圆圆的挂在你的应用墙上的时钟，可以实时看到当前时间哦~',
        img: 'https://tse2-mm.cn.bing.net/th/id/OIP-C.Au1TlzbTsavQuT10NnBjTwHaHa?w=180&h=180&c=7&r=0&o=5&pid=1.7',
        type: 'component',
        category: '小组件(测试)',
        url: 'https://www.xiaotao2333.top:5885/common/SnippetHtml/179'
    }
]

export const categorys = [
    '推荐应用',
    '实用工具',
    '科技资讯',
    '程序员集',
    '我的应用',
    '小组件(测试)'
]
